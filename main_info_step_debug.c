...
int Pop_Calcu(int *step, long long int *stack_num, int *idx_num, char *stack_opr, int *idx_opr){
...
long long int Calcu_Cell(int *step){
    ...
    Stack_Printer(step, Stack_Num, idx_Num, Stack_Opr, idx_Opr, "init");
...
int main(){
    ...
    int Step = 0; // count the operation times changing the Stacks(Stack_Num & Stack_Opr) --> equals to how many time "Stack_Printer()" called
